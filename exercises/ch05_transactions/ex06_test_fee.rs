...
pub fn exercise() {
    ...
    section(&format!("txid {:?}", &transaction.txid()));
    show_pretty_print(&transaction);

    section(&format!("txid {:?}", &transaction_d1c789a9.txid()));
    show_pretty_print(&transaction_d1c789a9);

    section(&format!("Fee for {:?}", &transaction.txid()));
    ...
    let fee = transaction.fee(retrieve_transaction_d1c789a9).unwrap();
