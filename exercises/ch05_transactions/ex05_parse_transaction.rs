use programming_bitcoin_in_rust::*;
use crate::util::println_exercise::*;

use bitcoin::transaction::Transaction;
use util::byte_string::ByteString;
use util::hexadecimal::hexadecimal_string;

pub fn exercise() {
    prompt("Transaction parsing");

    let txn = "";

    let mut txn_bytes: Vec<u8> = vec![];

    txn_bytes.resize(txn.len() / 2, 0_u8);
    hexadecimal_string(txn, &mut txn_bytes).unwrap();

    let transaction = Transaction::of(&txn_bytes);

    show_pretty_print(&transaction);

    section("2nd input script (signature unlocking script)");

    show_display(&transaction.inputs[1].script);

    section("1st UTXO script (P2PKH ScriptPubKey)");

    show_display(&transaction.utxos[0].script);

    section("2nd UTXO amount (satoshi units)");

    show_display(&transaction.utxos[1].amount);
}
